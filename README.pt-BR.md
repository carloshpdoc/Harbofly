...
- **Auto-discovery**: varre `~/Development` procurando artifacts de build de 20+ stacks — `node_modules`, `.build`, `target` (Rust/Maven/SBT), `.gradle`, `.dart_tool`, `vendor` (Go/PHP), `bin`/`obj` (.NET), `_build` (Elixir), `.terraform`, `.godot`, `zig-out` e mais (nomes genéricos só contam com o arquivo-marcador do ecossistema ao lado, então suas pastas nunca são tocadas), mais 25+ locais de cache conhecidos — package managers (npm, pnpm, Bun, Gradle, Maven, Cargo, Go, Flutter, pip, uv, CocoaPods, Homebrew…), editores (VS Code, Cursor, JetBrains) e **ferramentas de IA** (pesos de modelo do Ollama, Hugging Face, LM Studio). Zero config de path.
...
- **Desinstalador com "última vez usado"**: lista os apps instalados com a pegada completa (bundle + todos os resíduos que ele espalha) e **quando cada app foi aberto pela última vez** — os que você não toca há meses sobem. Ordena por tamanho, uso, fabricante ou categoria, busca por nome e desinstala (pra Lixeira, restaurável).
- **Gerenciador de itens de inicialização**: veja o que sobe sozinho no login e em segundo plano (Docker, updaters, agentes…) e **desative o excesso** — reversível (o agente é movido de lado, não apagado; reative quando quiser). Daemons de sistema/root aparecem só-leitura.
- **Idade em cada item**: cada cache/artifact mostra há quanto tempo não é tocado — dá pra ver de relance o que está abandonado.
- **Busca** nas listas de limpeza e de apps.
...
- **7 idiomas**: Português, English, Español, Français, Deutsch, 中文, 한국어 — com troca instantânea.
