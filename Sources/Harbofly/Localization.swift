...
struct L {
    ...
    var analyticsDetail: String { s("Só eventos, tamanhos e categorias genéricas de cache (DerivedData, node_modules…), com modelo do Mac, versão do macOS e idioma. Nunca nome, arquivo, caminho, projeto ou IP. Open source e auditável.", "Only events, sizes and generic cache categories (DerivedData, node_modules…), plus Mac model, macOS version and language. Never a name, file, path, project or IP. Open source and auditable.", es: "Solo eventos, tamaños y categorías genéricas de caché (DerivedData, node_modules…), con el modelo del Mac, la versión de macOS y el idioma. Nunca un nombre, archivo, ruta, proyecto ni IP. Open source y auditable.", fr: "Uniquement des événements, tailles et catégories génériques de cache (DerivedData, node_modules…), avec le modèle du Mac, la version de macOS et la langue. Jamais de nom, fichier, chemin, projet ni IP. Open source et auditable.", de: "Nur Ereignisse, Größen und generische Cache-Kategorien (DerivedData, node_modules…), dazu Mac-Modell, macOS-Version und Sprache. Nie Name, Datei, Pfad, Projekt oder IP. Open Source und prüfbar.", zh: "仅包含事件、大小和通用缓存类别（DerivedData、node_modules…），以及 Mac 型号、macOS 版本和语言。绝不含名称、文件、路径、项目或 IP。开源可审计。") }
